...
**489 tests, 0 failures.** 165 engine tests + 324 E2E tests across two device profiles.
...
### E2E — `tests/e2e/` (145 × desktop + phone)
...
| **THE RECKONING** | never fires on rounds 1-2, where the measured median deck is already 4.1x ahead and the tutorial runs; fires past three times the target from round 3; never drafts the Ordeal the round already has; is the same on the same seed; appears in the shape the Bookseller quotes, so the shop cannot promise a round the player will not get; the ceiling is measured under the rule it drafted; and measuring a future round never disturbs the one being played |
| **THE FORFEIT** | always hands back a discard first, so it can never meet THE DROUGHT and leave a round nobody can take a turn in; the round starts owing a word and a discard settles it |
...
| The menu | covered — one door to difficulty, sound, the rules and your run history; every door out of it comes back to it; the board still shows a muted game; it fits a phone |
| The main menu | covered — it is what you land on, PLAY is reachable at five viewports, the board cannot be touched through it, a first run is started on APPRENTICE and a returning one is not, and every door on it comes back to it |
| Round over | covered — the board is dimmed and blurred behind the stamp, and comes back after it |
