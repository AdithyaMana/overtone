...
| **THE RECKONING** | never fires on rounds 1-2, where the measured median deck is already 4.1x ahead and the tutorial runs; fires past twice the target from round 3; never drafts the Ordeal the round already has; is the same on the same seed; appears in the shape the Bookseller quotes, so the shop cannot promise a round the player will not get; the ceiling is measured under the rule it drafted; and measuring a future round never disturbs the one being played |
